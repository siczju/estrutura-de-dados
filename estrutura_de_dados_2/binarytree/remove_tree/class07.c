...
int buscar(No *raiz, int chave)
{
    ...
    else
    {
        ...
        { // vou para esquerda ou para direita
...
No *remover(No *raiz, int chave)
{
    ...
    {
        if (raiz->conteudo == chave)
        { // o valor pra remover está no nó?
            if (raiz->esquerda == NULL && raiz->direita == NULL)
            { // ver se não possui nenhum filho (folha)
                free(raiz);
                return NULL;
            }
            else{
        ...
        {
            if (chave < raiz->conteudo) // ver se o valor eh menor ou maior que a o valor da raiz
